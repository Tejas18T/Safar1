...
const backendUrl = "http://localhost:8200/admin/getAllActiveCompanies";
const deleteUrl = "http://localhost:8200/admin/deleteCompany";
...
const companiesReducer = (state, action) => {
  switch (action.type) {
    ...
    case ACTIONS.DELETE_COMPANY:
      return {
        ...state,
        companies: state.companies.filter((company) => company.userId !== action.payload),
      };
...
const SuspendCom = () => {
  ...
  const deleteCompany = async (userId) => {
    ...
    console.log("suspend company with userId:", userId); // ✅ Debug Log
    ...
    try {
      ...
      dispatch({ type: ACTIONS.DELETE_COMPANY, payload: userId });
    } catch (err) {
